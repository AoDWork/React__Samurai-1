...
{/*    ====    01. Как проходить курс     ====
    ...
    На верстку забиваем, верстаем - "абы було", в уроках изобретаем свой redux(24-41) потом изучаем обычный(42), также изобретам свой
...
{/*    ====    88. pure function (чистая функция)    ====
    ...
    1. Иммутабельность(неизменяемость) - чистая ф-я не должна мутировать(изменять) данные которые в нее пришли(props).
        Так делать нельзя потому что в пропсах приходит ссылка н объект, тоесть он храниться где-то за пределами ф-ии
        и при изменении этих данных в ф-ии соответственно они изменяться и в месте хранения(также это уже сайд эффект).
        Может только использовать эти данные для выполнения какого то алгоритма и выдать какой-то результат(jsx - если компонент).

        На примере users редюсера - это тоже чистая ф-я которая принимает state и action  и возвращает копию state, его изменять
        нельзя потому что еще какие то компоненты могут его использовать, также и с action нельзя его изменять потому что он
        может "прыгать" по другим редюсерам.



    2. Раз ф-я не может изменить данные которые в нее передают, значит она должна что-то вернуть, результат работы(копия state
        если говорим о reducere, таким образом сообщаем что state изменился). У чистых ф-й нету state и они не работают с 
        глобальными переменными.



    3. Нет сайд эффектов. Например запрос на сервер мы делаем только из санок, а не из компонентов или редюсеров, санки - не 
        чистая ф-я, и они не возвращают результат(явно через return). Не все ф-и в приложении должны быть чистыми.
        Чистыми должны быть - компоненты, редюсеры, селекторы.



    4. Детерминированность( предсказуемость, идемпотентность ). При одних и тех же входных данных ответ из чистой ф-и будет 
        одиноковый.
...
{/*    ====    89. Тесты, jest, tdd, тестируем reducer    ====
    ...
    Модульное тестирование(юнит тест) - тестирование отдельного компонента(ф-и, редюсер, селектор, чистый компонент) без
        связей с другими компонентами, меньше вероятность что протестированные аким образом компоненты при запуске полной 
        цепи выдадут неправильный результат. 


    Рассмотрим на примере Редюсеров. Они из мира бизнеса(state) и можно без UI компонента, диспатчей, коллбеков протестировать
        как работает редюсер передавая ему state и ожидая предсказуемого результата. Таким образом можно протестировать с 
        помощью кода без построения UI компонента и остальных связей - ускоряя разработку, а если не ускоряет, то делает процесс
        дальнейшего дебаггинга более быстрым и предсказуемым. Также тест - это некая инструкция о том как работает данная ф-я.



    Создадим тест для  profile-reducer, положим его в эту же папку.  profile-reducer.test.js - в настроеной системе вебпака говорит
        о том что это тест, настройки скрыты в реакт скриптс, и при запуске npm run test - будут запускаться тесты. Также для того
        чтобы система видела что это тест нужно код обернуть в конструкцию it. В ней добавляется описание что делает тест.

        Будем тестировать добавление поста ADD_POST который диспатчим с помощью action криейтера  addPostActionCreator.
        Для этого создадим переменную action и вызовем в ней action криейтер, в который должны передать текст сообщения.
        
        Теперь этот action диспатчим в newState и туда же передадим state - скопируем его из редюсера и удалим ненужные сейчас поля 
        которые все равно сейчас не изменятся.

        Тест состоит из 3х стадий: 1 - готовим исходные данные, 2 выполняем действие, 3 сравниваем результат действия с нашим 
        ожиданиями, тут мы ожидаем что после выполнения кода - в новый state добавиться новый пост и длинна массива таким образом
        должна равняться 5.  expect - импортировать не нужно, его добавляет jest(вроде). На против этой ф-ии у автора в вебшторме
        есть значек запуска.
        ...
        Еще можно сделать проверку на то что в постах добавился пост именно с тем тектом который мы передали. Можно было бы
        добавить еще expect в эту же проверку, но лучше делать эти проверки отдально, чтобы если будет ошибка было понятнее
        гд искать. И так как у нас два куска кода почти дублируются то вынесем общие переменные чтобы этого избежать.
    ...
    Есть тесты на TDD - test drive development - тест на тот код который еще не используется в проекте. Например мы хотим не только
        добавлять посты, но и удалять, пока никакого кода для этого нету и даже в UI компоненте никаких кнопок. Но можно написать
        такой редюсер протестировать его и добавлять - зная что логика уже работает.

        В тесте добавляем код для проверки, запускаем action криейтер deletePost с id - 1 сообщения которое нужно удалить. Потом
        происходит сверка длинны массива. 
        ...
        Тест не проходит потому что нету такого action криейтера, теперь допишем его в 
        reducer.
        ...
        Теперь тест работает корректно.


        Сделаем еще тест - защита от дурачка, если отправлен некоректный айдишник то длинна массива не должна измениться.
    ...
    Если сделать такие тесты для всего приложения и при запуске через npm они запускаются все, мы можем увидеть что дописав 
        например какой то код и он пройдет свою проверку, но в это же время другой тест не пройдет потому что мы что то сломали.
        Чем больше тестов для приложения тем лучше наглядность и не запуская приложение можно сразу в среде разработки 
        видеть где  и что сломалось.
...
{/*    ====    90. redux-ducks рефакторинг    ====
    ...
    redux-ducks - этот подход говорит что не нужно черезмерно дробить код в редюсерах, тоесть action, action creators, reducer -
        должны быть в одном файле, чтобы не запариваться с импортами-экспортами, чтобы было наглядно и можно было быстро изменять
        код и разрабатывать приложение.


    Большая проблема может возникнуть если совпадут названия action в разных редюсерах. Например если новый разработчик добавил
        новый редюсер и назвал action SET_USER_DATA, а так как редюсеров много он не смотрел их все и не запоминал какие там
        названия actions. И при диспатче action SET_USER_DATA Store раскидывает его по всем редюсерам, потому что он не знает
        какому редюсеру нужен этот action, таким образом выполнится код и в старом редюсере и в новом, а логика и код там
        совершенно разный, потому что названия actions совпадают. Чтобы такого не происходило redux-ducks рекомендует перед 
        названием actions еужно ставить как минимум название редюсера, или даже название проэкта.


        Зарефакторим код в auth-reducer

        было const SET_AUTH_USER_DATA = 'SET_AUTH_USER_DATA';

        стало const SET_AUTH_USER_DATA = 'auth/SET_AUTH_USER_DATA'; или = 'samurai-network/auth/SET_AUTH_USER_DATA'
...
{/*    ====    91. chrome extensions для react и redux    ====
    ...
    Рассмотрим расширения для хрома которые помогают в работе. 

    //! Обычные расширения которые установлены для пользования интернетом, такие как адблок могут вызывать конфликты при 
    //! разработке приложений, рекомендуется создать отдельного юзера в браузере для того чтобы использовать этот акк только для 
    //! разработки или пользоваться в режиме гостя если на чужом компьютере.



    1. React Developer Tool. Добавляет в панели разработчика  вкладки:

        Components - показывает всё дерево компонентов(клик на компоненте показывает -  props, state, hooks и последовательность 
                     рендера)
            и 
        Profiler - записывает скорость рендеринга компонент



    2. Redux dev tools. Сразу не работает, его нужно настроить. Запускается с иконки - перекидывает на гитхаб с инструкцией по 
        настройке.

        Используем способ 1.2. advanced store setup. В redux-store допишем строчки заменив старый метод создания store, 
        + импортируем compose
        ...
        теперь значек расширения стал зеленый и пошел коннект с проектом.
...
{/*    ====    92. тестируем компоненты, тесты, react-test-renderer    ====
    ...
    Есть компании в которых есть только компоненты, их тоже нужно уметь тестить в отрыве от бизнеса. У нас уже есть 1 тест
        ...
        Тут нету expectation, мы просто смотрим чтобы код выполнился без ошибок. Но этот тест не проходит у Димыча потому что
        App оборачивается withRouter + connect и для рендра нужны данные из store, и чтобы все работало в App мы его обернули
        снаружи в index - BrowserRouter + Provider  и получается что в самом App не хватает этих ф-й(она не полноценная)

        Чтобы этот тест прошел нужно сделать для App контейнерный компонент, поместим его тут же в App. Вместо экспорта апп
        с обертками получим ее в аппКонтейнер который отрисуем в SamuraiApp и обернем тем что было в index.js перенеся импорты
        и уже SamuraiApp экспортируем.
        ...
        В тесте меняем App на SamuraiApp.
        

    //! У меня по умолчанию тест другой 
    ...
    //! Для теста компонентов нужны тестовые фреймворки. Их много библиотек, фреймворков и подходов. Jest подключен по умолчанию
    //! это среда запуска тестов, а для способов тестирования и разных рюансов уже подключаются различные библиотеки. Для 
    //! актуальных тестов на 2019 год Димыч гуглил статью по тестированию компонентов - testing react components: the mostly 
    //! definitivw guide (2019) - прямо всё из этой статьи делать не будем, возьмет библиотеку react-test-renderer
    //! npm i react-test-renderer@16.8.6 --save-dev  
    //! @16.8.6 - означает установить такую версию, версия renderer и react должны совпадать иначе может не работать. 
    //! -dev  означает что эта зависимость установится только для разработки(development) и при разворачивании приложения на
    //! сервере она не будет включаться в общий пакет модулей, таким образом уменьшая вес приложения.
    //! этот модуль позволяет виртуально отрендерить компонент чтобы в нем проверить нужные нам ожидани(поискать теги, посмотреть
    //! state).
...
{/*    ====    97.  обновление профиля    ====
    ...
    Создадим инпуты для контактов. key подставляем чтобы задать плейсхолдер, //! а для имени поля передадим "contacts." + key - 
        ...
        Допишем в profile-reducer блок else для обработки ошибки и вытягивания из ответа сообщения. 
        ...
        Теперь ошибка задиспатчится и попадет в redux-form под словом error. Текст ошибки - invalid url format (Contsacts=>Facebook)
        //! но если нужно показать ошибку конкретно под полем в котором ошиблись то можно написать так(а еще можно было бы 
        //! распарсирь ответ и  вытянуть из него само название ошибки и показать вместо текущей ошибки только название)

            dispatch(stopSubmit("editProfile", { "contacts": {"facebook": response.data.messages[0]} }));
        
            
        добавим в ProfileDataForm(посмотрим как реализовывали в Login)> покжем ошибку после кнопки
    ...
    Но сейчас ошибка показывается только если не происходит выход из режима редактирования в ProfileInfo (закоментировали в onSubmit - 
        setEditMode чтобы не срабатывал). 
        //! Так как у нас в profile-reducer асинхронная ф-я saveProfile - возвращает промис, мы можем обработать его стату в
        //! ProfileInfo. При положительном результате(resolve) будет выполняться ф-я(ф-я для того чтоды передать false ???)которая 
        //! сменит setEditMode на false и закроется режим редактирования. Но если из saveProfile прийдет статус ошибки (reject)
        //! setEditMode не измениться и мы останемся в режиме редактирования и увидим ошибку. 
        ...
        //! Но чтобы обработать reject из промиса нам нужно егожвернуть из profile-reducer
        
                dispatch(stopSubmit("editProfile", { _error: response.data.messages[0] }));
                return Promise.reject(response.data.messages[0]);


        //! но вообще так делать неправильно, потому что UI не должен ждать ответа после диспатча, он должен задиспатчить в 
        //! что то и забыть, а если потом этот диспатч приведет к изменению state тогда уже отрисоваться под новый state.
        //! можно было бы сделать это по другому, результат ответа поместить в state и в зависимости от него в onSubmit уже
        //! выключать режим редактирования
...
{/*    ====    99.  try-catch, router switch, classnames    ====
    ...
    Service Worker в index.js это для PWA.

    react - это скрипты и превращение в виртуал дом. А react-dom - рендерит виртуал дом в реальный дом.


    //! Provider помещает store в context так все дочерние компоненты могуть быть consumers - потребителями, тоесть
    //! использовать его. В Provider нужно помещть только глобавльные вещи, например store, темная/светлая тема, локализация. 




    Switch используется для того чтобы при первом же совпадении имени отрисовать именно этот компонент. Он импортируется из
        react-router-dom.

        Сейчас если есть такой случай, когда попадаем на логин форму по ссылке с фейсбука, то браузер отрисует и див - фейсбук 
        и компонент Логин потому что есть частичное совпадение пути
        ...
        а нам нужно чтобы показывался только див - фейсбук, то нужно прописать exact для точного совпадения, и тогда логин 
        компонент будет показываться только тогда когда будет url  /login ,без всяких хвостиков дальше

            <Route exact path="/login/facebook" render={() => <div>Facebook</div> } />


        Switch работает так что при точном совпадении имени он отрисует этот компонент и дальше не будет смотреть, тоесть все
        специфицеские url нужно переместить вверх а все общие вниз.
            ...
            при такой записи свич найдет первое совпадение с /login/facebook - отрисует компонент для него и остановиться.
            
            
        //! если точного сопадения не найдено то будет показан эл, с частичным совпадением.
        //! можно сделать  кейс по дефолту *, если не найдеться ни одного совпадения даже частичного тогда будет показан он
        ...
        //! Switch не исключает exact. Например мы хотим чтобы на главной показывался профиль, а сейчас показывается 404
        тогда сделаем exact, потому что в противном случае будет показан 404 при корневом пути.

            <Route exact path="/" render={withSuspense(<ProfileContainer />)} />


        теперь получается что и для корня сайта и для профиля получается дублирование компонента, сделаем просто
        редирект на компонент профился при попажании на корневой url. Redirect импоритруем из react-router-dom
        
            <Route path="/" render={ ()=> <Redirect to={"/profile"}/>} />


        //! карта движения по url и что будет показываться для корня сайта обычно говорит заказчик.




    Библиотека classnames - Димыч использовал в paginator. Для того чтобы присвоить одному элементу два класса без
        всяких изгаляний  такого вида `${} ${}` или style.class1 + ' ' + style.class2. 
        
        тут пишеться испорт библиотеки и в скобочках через запятую перечисляются классы которые нужно добавить
        
            import cn from "classnames"

            ( cn(style.class1, style.class2) )


        Можно делать например вот так. Тут styles.pageNumber - переданное вторым примениться обязательно, а для styles.selectedPage
        задаем логику и берем его в квадратные скобки - как строку, потому что передавая объект там не может быть переменной.
        Тоесть присвоим styles.selectedPage(жирный шрифт) тому элементу который выбран сейчас с измененным именем(потому что моуль ксс так меняет
        имена классов - инспект в браузере на элементах Paginatora). Так как мы в map то этот стиль примениться к элементу только
        currentPage(текущая выбраная страница не меняется и приходит в props)  будет равен === p (это имя с кракозябрами)
        ...
        //! Есть еще более крутая штука - styled component, его планирует рассмотреть в следующем курсе.




    Обработка ошибок try catch. Рассмотрим при изменении staus юзера(есть лимит сервера для этого поля если он превышен будет 
        ошибка, но она никак не показывается и юзер об этом не знает, может быть не ошибка, а 200 ответ но в резалт коде будет 1
        тогда можно вытащиться сообщени об ошибке). Эта ошибка не стопает приложение потому что она в асинхронном запросе. Можно
        решить локально конкретно для промиса(запроса) или отлавливать все ошибки, если ошибку обработали локально то в глобально
        она уже не попадет.

        1. Глобальный отлов ошибки. В App в componentDidMount навесим на глобальный объект window обработчик событий, для отлова 
            ошибки при котором будет выполняться уведомление юзера методомcatchAllUnhandledErrors. Тогда при componentWillUnmount 
            нужно убрать за собой этот обработчик, потому что это не реактовский а нативный метод, реакт его не уберет.
                ...
                componentWillUnmount() {
                    window.removeEventListener(this."unhandledrejection", this.catchAllUnhandledErrors)
                }


        2. Обработка локальной ошибки. Смотрим в api updateStatus возвращается из profile-reducer, и помимо положительного
            рез-та может вылезти ошибка. Тоесть пишеться try выполняется код и если ошибка то выполняется catch.
            //! в catch можно не только уведомить юзера об ошибке, можно задиспатчить ошибку, откатить изменения которые были  
            //! до запроса(например если отправляем несколько запросов типа - забронировать место, забронировать второе место,
            //! подтвердить оплату...) для таких сложных случаев используют вместо санок - saga (библиотека для управления 
            //! ajax запросами / асинхронными операциями) 
                ...
                export const updateUserStatus = (status) => async (dispatch) => {
                    try {
                        ...
                        alert(error) - тут можно достать месседж показать или раскрыть статус и посмотреть если допустим статус
                    }                   500 то сказать что это серверная ошибка
    ...
    //todo прочитать про PWA. serviceWorker  нужен? Прочитать про try catch
    //! Димыч заменил хеш роутер на браузер роутер, разворачивают приложение на Хироку и там полноценный сервер, там не нужен хеш
    //! Поменять в package - сайт для деплоя, сохранить его в профиле на сервере камасутры.
    //! Продолжение курса реакт самурай будет на другом канале - SamuraiJS, ссылка под видео.
...
{/*    ====    100. Теория ReactJS + Redux за 90 минут    ====
    ...
    Хуки как и хоки и классы с наследованием позволяют не дублировать код и єффективно его переиспользовать. Они начинаются на 
        use - useState, useEffect. Можно написать свой хук но тоже назвать его с начала на use чтобы было понятно что это хук.



    React - библиотека которая позволяет эффективно отрисовывать ХТМЛ, тоесть не перерисовывает те элементы которые перерисовывать не
        надо, те элементы которые остались без изменений не перерисовываются.

        До появления SPA applications нам приходилось взаимодействовать с DOM деревом напрямую. При попадании хтмл в браузер, браузер
        парсит его и на базе каждого тега создает сложный объект со множеством свойств, тоесть документ с тегами превращается в
        объектную модель(совокупность объектов). При изменении одного объекта всё дом дерево перерисовывается, что занимает очень 
        много ресурсов процессора и не эффективно. Или можно точечно перерисовывать какой то обхект но для этого нужно писать много
        дополнительного кода(?//todo почитать есть ли такое)
        
        Поэтому в реакте придумали virtual DOM это реактовская модель объектов с упрощенными свойствами, тоесть объекты получаются
        простые и их создавать намного быстрее и менее затратно. Он строиться на основе возвращенной разметки из компонентов(JSX). 
        При изменении создается новый виртуал ДОМ и сравнивается со старым и уже на основе получившегося результата перерисовываются
        только те объекты в реальном ДОМ в которых произошло изменение.



    Компоненты - выдуманные нами блоки(кастомные теги <UserProfile />) для того чтобы раздробить большое приложение на небольшие
        части которыми легко управлять и переиспользовать, внутрь мы можем передавать данные или коллбеки(функции). И задача такого
        компонента вернуть разметку JSX. Но такую разметку браузер не понимает(она используется для более понятного восприятия 
        человеком кода - это JS код который выглядит как HTML разметка) и поэтому JSX транспилируется Babel в код JS для того чтобы
        реакт в браузере с ним работал (реакт загружается в браузер).


        Компонент всегда называется с большой буквы. Есть классовые и функциональные компоненты.

        У компоненты может быть свое хранилище данных(local state) для классовых - this.setState, для ф-х используется хук useState.
        Этими данными компонент может делиться с дочерними, по умолчанию они собираются в объект props и передаются в компонент где
        нужно из этого объекта достать нужные данные. 


        Компоненты могут быть вложенными друг в друга, один отрисовывает внутри себя другой, таким образом получается дерево
        компонентов. Родительский компонент передает в дочерний данные для того чтобы дочерний компонент мог отрисоваться
        по другому, с другим наполнением или мог выполнять какое-то действие. 
        
        <User user={this.state.user} > - в компонент User передается данные по имени user которые взяты из state (this - контекст
            для классовых компонентов)


        Таким образом возвращаемый JSX может быть разным в зависимости от входных данных(props) или от данных которыми владеет 
        комопонет(при изменении local state).


    //! Life cycles(методы жизненного цикла) 
    Каждый раз при каком то изменении Реакт запраивает у всех компонентов JSX разметку и строит виртуал ДОМ, но этот процесс
        тоже можно оптимизировать для еще большего быстродействия и меньшего потребления ресурсов.  Можно сделать так чтобы
        компонент сам сравнил новый он даст JSX или нет, и если нет то он даже не будет давать эту разметку и тогда новый виртуал 
        ДОМ может не строиться(если изменились данные которые не влияют на разметку). 

        Для классовых компонентов это реализуется методами жизненного цикла. На базе классового компонента реакт создает в памяти
        объект с которым может взаимодействовать. 
        
        Когда создается объект, для генерации JSX вызывается его метод render, на базе этого JSX строиться виртуал ДОМ, далее этот
        компонент вмонтируется в реальный ДОМ и Реакт сообщает компоненту что он вмонтирован componentDidMount(), мы можем дописать
        в этот метод свое поведение которое выполниться в момент после монтирования, таким образом можно в различные моменты
        жизненного цикла объекта выполнять те действия которые нам нужны(сайд эффекты: асинхронные операции - запросы на сервер, 
        сеттаймауты; обращения напрямую к ДОМ элементам, ... ).

        У родительского компонента изменился state и Реакт начал перерисовку всего дерева, но допустим у дочернего компонента ничего
        не поменялось - не пришли новые props(или компонент вообще их не использует их нет) и нету(не изменился) local state, тоесть 
        его не нужно перерисовывать, в таком случае  Реакт перед вызовом render может запустить метод shouldComponentUpdate() - должен
        ли вообще перерисоваться этот компонент, в этот метод Реакт передает новые props и state, если state или props у этого 
        компонента нету то они будут ничем, а если есть то они сравниваются с предыдущими и если нету изменений возвращается false и
        тогда метод render не вызоветься и компонент в виртуал ДОМ не будет строиться заново/обновляться.

        В ф-х компонентах это реализуется с помощью React.memo


        PureComponent(пьюр) - чистый КЛАССОВЫЙ компонент в котором уже изменен метод shouldComponentUpdate, он автоматически проверяет 
        нужно ли заново отрисовывать компонент. Сравнение проводиться не глубокое, потому что это очень затратная операция, поэтому
        некоторые изменения в state и props могут остаться проигнорированными(в случае если объект мутирует, если сделать 
        иммутабельно то будет каждый раз рендер даже если данные не изменяться, потому что ссылка будет другая на новый объект)


    Компонент смонтировался и реакт сообщил это вызвав метод componentDidMount, на последующие рендеры в этом компоненте вызывается
        уже метод componentDidUpdate. Так как он в отличие от маунт будет срабатывать при каждом рендере мы тоже можем в нем делать
        сайд эффекты, но уже с УСЛОВИЕМ (например если в этом методе сделать изменение state то будет постоянно срабатывать рендер
        потому что shouldComponentUpdate будет постоянно true так как ссылка не объект state будет каждый раз новая).

        

    //! Redux  
    Компонент возвращает JSX разметку в зависимости от входных данных, итого данные - первичны(главные). Чтобы данными эффективно
        управлять а не размазывать по всем покмпонентам(хранить в разных компонентах) нужно поднять эти данные на самый высокий
        уровень(компонент) и уже оттуда управлять. Но само управление данными в реакте не удобно(setState никуда не годится, с
        хуками useReducer useDispatch уже получше(это Redux в Реакте) но все равно не то) поэтому используем стороннюю библиотеку
        по state менеджменту, одна из самых популярных - Redux.

        
        Redux состоит из одного главного объекта - Store(хранилище), в нем хранится общий _state и редюсеры. 
        
        Наша задача создать этот store, его мы создаем комбинируя(объединяя) несколько редюсеров в каждом находиться свой state.
        При combine reducers (объединении) - засовываем в него редюсеры из которых он будет состоять. 
        
        Редюсер это ф-я(преобразователь) его задача при получении на входе action(объект у которого есть как минимум тип ) и своей 
        части state из общего _state - если нужно преобразовывать свой state ИММУТАБЕЛЬНО(не изменяя входящий state, создает его 
        копию и работает с копией). action отправляется на все редюсеры сразу,  редюсер уже решает нужно ли ему реагировать на 
        него(если тип action который пришел  и тип который в редюсере совпадет тогда реагирует  изменяя state по прописанному в 
        редюсере в конструкции switch кейсу).


        Store умный объект с которым мы взаимодействуем через интерфейс(методы и свойства) store. У него есть 3 главных метода
            
            - store.getState() - позволяет получить актуальный _state из store
                
            - store.subscribe(subscriber) - позволяет сабскрайберу подписаться на изменения _state. Для сабскрайбера это всегда
                        ф-я которая передается внутрь store и когда _state измениться store вызовет(уведомит?) подписчика и в этой 
                        ф-ии мы сможем взять актуальный _state через getState и сделать еще какие то действия
            ...
            - store.dispatch(action) - позволяет преобразовать state благодаря диспатчу action(инструкция). action - объект у 
                        которого есть как минимум тип, как бы инструкция для редюсера что ему сделать и нужные для этого данные.
    ...
    React и Redux две независимые библиотеки которые можно использовать и не в связке. Redux может работать и с angular и с view и
        с нативным JS. Для того чтобы redux было удобнее использовать в react создали библиотеку react-redux - именно из нее получаем
        connect, компонент provider и т.д.

        Напряжно работать со store в чистом виде, нужно прокидывать через компоненты, не забывать байндить, делать диспатчи и т.д.
        поэтому используем react-redux задача которой обеспечить взаимодействие компонента со store.  


        Как компоненту взять store. Благодаря connect - (HOC - high order component - компонент высшего порядка) - ф-я которая 
        принимает в себя комопнент и создает вокруг него контейнерный компонент(оболочку) для того чтобы этот контейнерный компонент
        снабдил наш(принимаемый) компонент данными и ф-ми из store

            connect(mstp, mdtp)(InputComponent)

        mstp и mdtp - эти ф-и должны передаваться в коннект вместе с входящим компонентом
        
        mstp - map state to props  - ее задача взять из всего state часть и передать в компонент под именем которое мы придумаем, 
                например у и эта часть будет доступна в компоненте в пропсах, чтобы ее достать обратимся props.y
        ...
        mdtp - map dispatch to props - ее задача взять метод dispatch из store и вернуть из mdtp объект с коллбеками которые
                компонент сможет вызвать и при вызове на самом деле произойдет какой то dispatch в бизнес скорее всего при этом
                измениться _state и компонент перерисуется локально (только тот компонент который законекчен и получит новый объект
                через mstp)

                mdtp = (dispatch) => {
                    return {
                        callback1 = (a, b) => {
                            dispatch(actionCreator(a, b))
                        }
                    }
                }


        //! Компонент контейнерный получает state для mstp из store через контекст(context). Контейнерный компонент подписывается
        на изменения state и когда state изменяется каждый раз контейнерный компонент будет вызывать mstp. Компонент берет state
        через метод getState потом выполняется поверхнострое сравнение полученного и предыдущего state и если они не равны 
        выполняется форс апдейт и компонент входной перерисуется.

                store.subscribe(() => {
                    const s = mstp(store.getState())

                    if( !shallowEquality(s, prevState)) {
                        this.forseUpdate()
                    }
                }
                )

        таким образом будет много подписок, на каждый контейнерный компонент и благодаря сравнению state они будут перерисовываться
        только тогда когда будет изменен именно их кусок state


        Для получения store из context, для этого оборачиваем app component в provider и отдаем ему store. Provider это тоже 
        компонент который берем из react-redux. 
        
                <Provider store={store}><App /></Provider>

        provider добавляет store в context с помощью context api. Если в родительском компоненте что то добавили в контекст то в 
        любом дочернем можно получить эти данные став consumerom(потребителем), таким образом этот store не нужно прокидывать
        через все компоненты в пропсах.

        //! не стоит использовать context для всех данных, только для глобальных, остальные данные передаем через props
        

        app - главный компонент в котором уже есть какой то код,  он отрисовывается в index компоненте (в индекс не пишут код кроме
        рендера апп, потому что он корневой, код при написании в нем может не автообновляться ))



    Мы должны диспатчить только action или санки(thunk). action это правильно упакованный объект для редюсеров и чтобы он сработал
        не должно быть опечатки ни в названии type или свойства, требуется передать нужное количество свойств. Поэтому для упаковки
        action объектов занимаются actionCreator, ф-и которые лежат рядом с бизнесом и занимаются упаковкой этих action.



    В mstp мы достаем из общего state его часть, и эта часть можеть иметь длинный путь(большую вложенность), могут применяться
        фильтраци, также бизнес(store) не должен хранить данные так чтобы UI было удобно их использовать, он хранит их удобно для 
        себя, поэтому такое доставание может быть не простым. И для того чтобы перевести такое доставание с последующим 
        использованием данных в удобный вид используются селекторы(это ф-и в которых уже настроен путь к нужной части данных и
        возможно реализованы фильтрации или другие операции с этими данными). Когда в mstp приходит state его отдаем селектору
        а он уже выбирает то что нужно и возвращает нам, таким образом селектор инкапсулирует в себе код выборки и делает mstp
        простым, без лишних знаний о структуре state.
        ...
        //! возникает проблема - внутри контейнерного компонента мы подписались на любые изменения state и даже если state 
        изменится у другой части не относящейся к нашему компоненту, всё равно запуститься mspt, а так как селектор может каждый 
        раз возвращать новый объект(ссылка новая хоть по сути значения могут быть такимиже как были, но поверхностное сравнение
        смотрит только на ссылки) если используется например filter или в селекторе может быть много расчетов которые нагружают 
        систему, то получается что компонент будет перерисовываться даже если ему это не нужно, чтобы такого избежать нужно 
        использовать библиотеку //! reselect. 




    И браузер(хром) и Node.js работают на движке v8 он понимает JS. Для браузера еще нужно отрисовывать хтмл, а node не нужно
        этого делать и так как он запускает JS вне браузера то можно взаимодействовать с файлами системы, писать сервер и т.д.
        Наше приложение SPA будет запущено в браузере но node нужен для того чтобы эффективнее разрабатывать(поднять сервер, 
        установить необходимые библиотеки, атоматизировать сборку проекта, ...)



    Компоненты: классовые и ф-е. Реакт стремиться к ф-му программированию поэтому классовые отходят на второй план.
        Ф-й компонент - ф-я которая принимает props и возвращает jsx. Хоть это и ф-я но мы не вызываем ее напрямую, за нас
        это делает React когда нужно.

        Компонент может принимать аттрибуты(данные). Когда реакт видит что тег отрисовывается с аттрибутами, упаковывает эти
        аттрибуты в объект props и передает компоненту чтобы в нем можно было ими пользоваться. props компоненту нужны для 
        отрисовки динамического (изменяющегося) jsx.

        Данные могут приходить из props или из локального state, в ф-х компонентах используются хуки для этого.



    Обычно в реакте используем map для получения из массива объектов - массива с jsx элементами, которые уже и будут отрисовываться.
        Обязательно при использовании map нужно задавать ключи - key(таким образом будет ускоряться построение порядка jsx элементов
        и они не будут путаться/конфликтовать между собой)



    Источник истины - state, все инпуты должны изменяться только через изменение state(или локального state) - flux круговорот, 
        поэтому нужно делать инпуты контролируемыми - указывать value/checked. Тогда input будет изменяться только при изменении 
        его значения в state и для этого нужно на onChange повесить обработчик событий чтобы он передавал в state данные которые 
        юзер впечатывает в input.



    Презентационный компонент - в котором нету локального state и side эффектов. Он изменяет выдаваемый jsx только от входящих
        props и удобен в тестировании и переиспользовании. Все данные и сайд эффекты для него получает контейнерный компонент
        вокруг него. Для разных компонентом может потребоваться одинаковые контейнерный компоненты, чтобы автоматизировать их
        создание используем HOC - ф-я высшего порядка, которая принимает пезентационный компонент и оборачивает его контейнерным.



    Чистая ф-я которая зависит только от входных данных и при одних и тех же данных всегда выдает один и тот же результат, потому
        что у нее нету локального state, и она не изменяет входящие данные, а если с ними нужно работать делает ГЛУБОКУЮ копию 
        данных и работает с ней.



    Interface сервера как программы - ендпоинты на которые мы отправляем запрос по определенным правилам (REST API):
        - тип запроса - GET POST PUT DELETE (CRUD) POST - create, GET - receive, PUT - update, DELETE - delete.
        - данные необходимые для отправки
        - получаемый ответ
        - возвращаемые http коды

        Делаем запрос с помощью axios библиотеки, возвращается промис(объект у которого 3 состояния - pending(выполняется), redolved
        успешно выполнен, rejected - ошибка выполнения ). В then передаем коллбек ф-ю которая сработает когда промис зарезолвился.
        Запрос операция асинхронная.

            axios
                .get("hhtps://blabla.com/users")
                .then( data => console.log(data) )
    ...
    Если компонент ф-й то реакт его вызывет как ф-ю и передает в него props. Если компонент классовый то на базе этого класса 
        создается объект и уже с ним взаимодействует реакт. Так как объект храниться в памяти то у него есть методы жизненного
        цикла + локальная память(локальный state).

            <Header name="samurai" />           -->     Header({name: "samurai"})       - ф-й, отрисовка сразу

            <Man name="samurai"  age={31} />    -->     let man1 = new Man({            - классовый, создание
                                                            name: "samurai", age: 31
                                                        })
                                                        let jsx = man1.render           - вызов метода отрисовки(render)

            
        Когда полученный из первого рендера jsx вставляется на страницу, вызывается метод жизненного цикла componentDidMount()
        который сообщает компоненту что он вмонтирован, в этом методе можно выполнить сайд эффект, последующие рендеры будут 
        вызывыть метод componentDidUpdate() - произошло обновление.



    Paginator. Сервер по запросу не выдает всю информацию, он ее делит на страницы, для получения страницы нужно знать как на 
        сервере это устроено и какой размер страницы, чтобы потом привести к нужному нам виду.



    Над презентационным компонентом может быть несколько контейнерных, например один получает доступ к store, внутренний отправляет
        запрос на сервер, и самый глубокий уже презентационный компонент который и отрисуется с уже полученными данными. При этом
        каждый компонент рендерит внутренний.



    Для того чтобы UI общался со store мы используем connect, но при появлении сервера появляется второй источник истины и с ним
        тоже нужно взаимодействовать с помощью HOC - withRouter(обращаться к серверу и получать из него данные). Он также создает
        вокруг целевого компонента контейнерный для взаимодействия с сервером(url).



    Login. Используем логинизацию с помощью куки - файл который храниться в браузере для каждого сайта свой, если мы уже 
        залогинились то в куки помещается сертификат по которому сервер определяет пользователя. Сертификат зашифрован его
        может расшифровать только сервер. При любом запросе на сервер браузер также посылает куки, расшифровывается сертификат
        и отдается сообщения/друзья именно этого пользователя.
        
        Когда впервые заходим на сайт нас отправляет на страничку логинизации потому что мы впервые тут(куки пустое).

        Мы делаем кросдоменный запрос, тоесть с локалоного хоста на котором запущено приложение мы делаем запрос на другой сайт,
        чтобы прицепилась куки нужно прописывать withCredentials 
    ...
    Thunk(санка). 
    Можно обойтись и без нее, но тогда нужно определиться где выполнять запросы на сервер. Тоесть делать запрос из
        контейнерного компонента из UI, так как диспатчить мы можем только action. Но это немного не правильно с точки зрения 
        архитектуры(когда проект будет становиться больше начнется путаница, а так все запросы будут в одном месте кода) и UI 
        становиться менеджером(посредником) между данными с сервера и бизнесом(получая данные с сервера диспатчит их в store),
        а это не хорошо, потому что UI должнен быть конечной точкой, которая получая данные возвращает jsx.

        Нужно чтобы бизнес делал запрос на сервер и в UI отдавал уже готовые данные, но в store только редюсеры - чистые ф-и
        которые не делают сайд эффекты. Поэтому создаются санки - ф-и которые принимают метод dispatch и внутри себя делает
        сайд эффект(асинхронную операцию) и множественные диспатчи.  
        
            const addPost = (dispatch) => {
                dispatch(onLoading())           - показываем крутилку
                axios.post({message}).then(() => {
                    dispatch(addPost(message))
                    dispatch(offLoading)
                })
        ...
        Для того чтобы санку можно было настроить с помощью замыкания используем санк криейтор, так в нее можно передать информацию.
        ...
        Санка хороша тем что ее тоже можно задиспатчить, таким образом общаясь с бизнесом только через диспатч. Чтобы видеть
        что пришла санка и она не должна идти в редюсер а должна выполниться нам нужно подключить middleware - промежуточный 
        слой который определяет action или санка и если action прокидывает дальше в редюсеры, а если санка то выполняет ее.
    ...
    HOC. 
    Позволяет переиспользовать логику. HOC создает контейнерный компонент в котором зашито определенное поведение.



    reduxForm, formik - библиотеки для работы с формами. Для того чтобы redux state не засорять временными данными из инпутов в форме,
        которые могут и не стать данными(юзер может стереть буквы) используются эти библиотеки которые в state создавали свою часть
        и работали только с ней(сейчас используют локал state/хуки без редакса) формально находясь в бизнесе но им не являясь.
