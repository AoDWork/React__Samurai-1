...
{/*    ====    74.  componentDidUpdate    ====
    ...
    В комментариях предлагали брать статус из профиля потому что он там уже будет, но автор хочет нам показать как работать с 
        ...
        реакт попадает в компонен он создает на основе класса объект и взаимодействует с ним. Получает от него jsx разметку,
    ...
    Этот метод будет вызываться каждый раз когда status не успевает первым обовиться. То есть сначала загрузился профиль и отрисовал
        ...
        //! значение из props и на следующий раз они будут равны и setState не сработает.
    ...
    Теперь мы всегда будем видить status. Но если перейти на какого то пользователя - видим его статус, попробуем средактировать,
        у нас статус обновиться, но на самом деле обновиться наш статус и он будет показываться в чужом профиле пока мы не обновим
...
{/*    ====    75.  redux form (React final form) - введение    ====
    ...
    //! Как оказалось React final form это тонкая Реакт обертка для Final form библиотеки(там уже есть создание формы) - нужно 
    //! нверное ставить обе библиотеки??? погуглить.
    ...
    //! Уточнить нужна ли final form для работы React final form - или эта библиотека может работать без нее.
    //! Установить react-final-form, удалить import в reducere - сделать через хуки компонент, удалить HOC.
