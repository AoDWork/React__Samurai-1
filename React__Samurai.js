...
{/*    ====    40. Практика - добавление сообщения     ====
    ...
    //! Не работает, при вводе текста в textarea выкидывает в консоль ошибку, а символ удаляется из поля, но заносится
        в state, хоть следующий раз и перезаписывается. Через ref тоже самое, оказалось что при работе в Profile  в 
        textarea выкидывается таже ошибка но символы при этом видны и накапливаются, а тут остается только последний
        введенные и его не видно. Видимо это из-за изменения принципа рендера самого реакта в новых версиях. 
