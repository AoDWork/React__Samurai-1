...
{/*    ====    45. Практика - connect, mapStateToProps, mapDispatchToProps     ====
    ...
    Проблемы которые будем решать:
        1) передача store через Context (мы создали свой Provider). Теперь можно пользоваться Provider из библиотеки, у него такой
            ...
            //*! Мы создавала контейнерные компоненты для того чтобы получить данные из store и коллбеки(ф-и которые обращаются к
            ...
            //*! Как єто работает. Ф-я connect создает контейнерный компонент в нем она рендерит переданный презентационный 
    ...
    //*! Приложение работает, но не впечатывается сообщение и не работают создания постов. Автор сказал подумать почему так, а
    на следующем уроке будем разбираться. Думаю потому что state теперь обновляется как-то по другому.
